...
int Thread::Get_Effective_Priority()
{
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) 
    {
	if (nextThread->Get_Effective_Priority() <= this->Get_Effective_Priority())
	{
		restore = this->Get_Priority();
                this->Set_Effective_Priority(restore);
